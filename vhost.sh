...
function install_wp {
    ...
    docker exec php83 wp core download --path=$site_doc_root --allow-root
    # 创建数据库配置文件
...
function create_site {
    ...
    # 自动创建站点配置文件
    cp -rf $DNMP_DIR/nginx/vhost.conf $VHOSTS_CONF_DIR/$INPUT_DOMAIN_NAME.conf
    # 替换域名
    ...
    # 修改文件权限 - 使用容器内的路径
    docker exec php83 chown -R www-data:www-data /wwwroot/$INPUT_DOMAIN_NAME
    # 重新加载nginx配置
...
function site_install_ssl {
    ...
    # 开始申请证书
    echo -e "${SB}开始申请证书${ED}"
    certbot certonly --webroot -w $VHOSTS_ROOT/$SITE_HOSTNAME/wordpress --email $CERTBOT_EMAIL --agree-tos --no-eff-email $domain_list_str
    if [ $? -eq 0 ]; then
...
function site_restore {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # 修改文件权限
            docker exec php83 chown -R www-data:www-data /wwwroot/$SITE_HOSTNAME
            # 输出成功
            echoCC "站点恢复成功"
        else
            echoRC "数据库恢复失败"
...
function install_phpmyadmin {
    ...
    # 修改文件权限
    docker exec php83 chown -R www-data:www-data /wwwroot/phpMyAdmin
    # 通过SQL文件创建数据库
